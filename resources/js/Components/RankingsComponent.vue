...
const teamRankings = computed(() => {
    return props.assignedTeams.map(team => {
        ...
        return {
            ...
            variation_points,
            total_points,
            wins: teamOverallResults.filter(r => r.is_winner).length,
            losses: teamOverallResults.filter(r => !r.is_winner).length,
            total_games: teamOverallResults.length + teamVariationResults.length,
            win_percentage: Math.round((teamOverallResults.filter(r => r.is_winner).length / 
                (teamOverallResults.length + teamVariationResults.length)) * 100)
        };
